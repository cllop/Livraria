...
public class RepositorioCaixaJDBC extends RepositorioJDBC implements RepositorioCaixa {
	...
	public void add(Caixa Caixa) {
		...
		try { 
			ps = con.prepareStatement(
					"SELECT * FROM perfilcaixa WHERE cpf=?"
					);
			ps.setLong(1, cpf);
			ps.execute();
	...
	public void remove(Caixa caixa) {
		...
		try {
			ps = con.prepareStatement(
					"DELETE FROM perfilcaixa WHERE id=?"
					);
	...
	public void update(Caixa caixa){
		...
		PreparedStatement ps;
		try {
			ps = con.prepareStatement("UPDATE perfilcaixa SET nome=?, sobrenome=?, nomeDeUsuario=?, rua=?, bairro=?, cep=?, numeroDoImovel=? WHERE id=?"
						);
	...
	public Caixa find(int id) {
		...
		try {
			ps = con.prepareStatement(
					"SELECT * FROM perfilcaixa WHERE id=?"
					);
			ps.setInt(1, id);
			ps.execute();
